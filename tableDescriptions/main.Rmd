...
for(ma in c("Alligator Harbor Aquatic Preserve","Apalachicola Bay Aquatic Preserve",
  ...
  in_nekton <- ma %in% unique(nekton$ManagedAreaName)
  in_coral <- ma %in% unique(coral$ManagedAreaName)
  in_sav <- ma %in% unique(sav$ManagedAreaName)
    ...
    for(indicator in websiteParams[HabitatName==habitat, unique(IndicatorName)]){
      ...
      for(i in 1:nrow(indicator_subset)){
        ...
        if(indicator %in% c("Water Quality", "Water Clarity", "Nekton")){
          if(indicator=="Nekton"){
            descText <- descTable[ManagedAreaName==ma & IndicatorName == "Nekton", DescriptionLatex]
            cat(glue("#### {parameter}"))
          } else {
            descText <- descTable[ManagedAreaName==ma & ParameterName==parameter & IndicatorName == indicator & SamplingFrequency == type, DescriptionLatex]
            cat(glue("#### {parameter} - {type}"))
          }
        } else if(habitat=="Submerged Aquatic Vegetation" & indicator=="Percent Cover"){
          descText <- descTable[ManagedAreaName==ma & HabitatName==habitat, DescriptionLatex]
          cat(glue("#### {parameter}"))
        } else if(habitat=="Coral/Coral Reef") {
          descText <- descTable[ManagedAreaName==ma & ParameterName==parameter & IndicatorName == indicator, DescriptionLatex]
          cat(glue("#### {parameter}"))
        } else {
          descText <- descTable[ManagedAreaName==ma & ParameterName==parameter, DescriptionLatex]
          cat(glue("#### {parameter}"))
        }
        cat("  \n")
        cat(descText)
