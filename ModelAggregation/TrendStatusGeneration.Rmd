...
disc_results <- openxlsx::read.xlsx("output/WQ_agg_results.xlsx") %>% as.data.table()
...
sav_results <- openxlsx::read.xlsx("output/all_SAV_MA_Results.xlsx") %>% as.data.table()
...
coral_pc_results <- fread("../../SEACAR_Trend_Analyses/Coral/output/PercentCover/Coral_PC_LME_Stats.txt") %>%
  as.data.table()
oyster_results <- fread("../../SEACAR_Trend_Analyses/Oyster/output/ManagedAreaName/Oyster_All_GLMM_Stats.txt")
...
oyster_results %<>% rowwise() %>%
  ...
  as.data.table()
# Check and assign coral trends via checkTrends function
checkTrends <- SEACAR::checkTrends
# Apply trend detection to coral results
coral_pc_results <- coral_pc_results %>% rowwise() %>% 
  mutate(Trend = checkTrends(p = LME_p, Slope = LME_Slope, SufficientData = SufficientData)) %>%
  as.data.table()
disc_model_results <- fread("output/disc_model_results.csv") %>% as.data.table()
MA_All <- SEACAR::ManagedAreas

##### Acreage setup
acreage <- openxlsx::read.xlsx("data/ChangeAnalysis_ChangeResultTable_2025.xlsx") %>% as.data.table()
...
# Add adjustments to ensure proper display in final output file
