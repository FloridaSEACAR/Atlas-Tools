...
# Merge discrete results and website parameters
disc_model_results <- left_join(disc, websiteParams %>% select(-Website), 
                                by=c("ParameterName", "RelativeDepth", "ActivityType", "SamplingFrequency")) %>%
  arrange(IndicatorName, ParameterShort) %>% 
  filter(Website==1) %>% as.data.table()
# Save results readout for reference within TrendStatusGeneration.Rmd
...
# Run in parallel, this will take some time
# In order to ensure the models are being run 'fresh': 
# delete/archive the previous model results in the output/models folder
...
# Perform hypothesis testing, determining increasing/decreasing probabilities
...
# Values with "IncreasingProbability" greater than or equal to 80% are designated positive (1)
# Values with "DecreasingProbability" greater than or equal to 80% are designated negative (-1)
# All else are designated as 0 = "no trend"
hypothesis_results <- hypothesis_results %>% mutate(
  Trend = ifelse(IncreasingProb>=0.8, 1, ifelse(DecreasingProb>=0.8, -1, 0))
)
# Write results into aggregate output file, used as input in TrendStatusGeneration.Rmd
openxlsx::write.xlsx(hypothesis_results, file = "output/WQ_agg_results.xlsx")

# Plot a barplot containing overview of trend results by indicator
...
plot <- ggplot(plot_data, aes(x = IndicatorName, y = n, fill = Trend)) +
  ...
  labs(
    x = "Indicator",
    y = "Count of Trends (# of Managed Areas)",
...
combined_plots <- list()
forest_plots <- list()
for(ma in MA_All$ManagedAreaName[1]){
  ma_abrev <- MA_All[ManagedAreaName==ma, Abbreviation]
...
## Using the following lines will allow you to observe the individual trends and the resulting "Pooled Effect" i.e. Aggregate trend for a given MA
# forest_plots[["Alligator Harbor Aquatic Preserve"]][["Water Clarity"]]
# forest_plots[["Alligator Harbor Aquatic Preserve"]][["Nutrients"]]

##### Further diagnostics if necessary for exploration
